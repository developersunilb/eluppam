...
const VowelLegoGame = () => {
  ...
  const treasureChestImageRef = useRef<HTMLImageElement | null>(null);
  const treasureChestOpenImageRef = useRef<HTMLImageElement | null>(null);
    ...
    vowels.forEach((vowelData, index) => {
      ...
      for (let i = 0; i < copies; i++) {
        ...
        do {
          ...
          const yRatio = (heapBaseY - blockY) / heapHeight;
          const currentWidth = heapWidth * (1 - yRatio * 0.8);
          ...
          for (const existingBlock of newBlocks) {
            ...
            if (distance < existingBlock.width) {
              isColliding = true;
              break;
            }
          }
  ...
  useEffect(() => {
    ...
    const animateMatching = useCallback(() => {
      setAnimatingBlocks(prev => {
        ...
        }).filter(anim => anim.block.opacity > 0);
      });
    }, [canvasSize.width, canvasSize.height]);
      ...
      if (clickedBlock) {
        ...
        setSelectedBlock(null);
            ...
            const handleMatch = (block1: Block, block2: Block) => {
              ...
              if (isProcessingMatch.current) return;
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-green-100 via-blue-100 to-purple-100 p-4">
      <div className="flex flex-col md:flex-row gap-6 max-w-7xl w-full">
        ...
        <div className="w-full md:w-80 bg-white rounded-2xl shadow-2xl p-6 border-4 border-purple-400 mt-6 md:mt-0">
