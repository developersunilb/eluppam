...
const MalayalamGridGame = () => {
  ...
  const [learningTime, setLearningTime] = useState(20);
  ...
  const [grid, setGrid] = useState<{ letter: string; color: string; row: number; }[]>([]);
  const [originalGrid, setOriginalGrid] = useState<{ letter: string; color: string; row: number; }[]>([]);
  ...
  }, [gamePhase, learningTime, playingTime, startPlaying]);
  ...
  const checkWin = (currentGrid: Array<{ letter: string; color: string; row: number; }>) => {
    const isCorrect = currentGrid.every((cell: { letter: string; color: string; row: number; }, index: number) => 
      cell.letter === originalGrid[index].letter
    );
    
    if (isCorrect) {
      const timeElapsed = Math.floor((Date.now() - playingStartTime) / 1000);
  ...
  const getCorrectCount = () => {
    return grid.filter((cell: { letter: string; color: string; row: number; }, index: number) => 
      cell.letter === originalGrid[index].letter
    ).length;
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100 p-8">
      <div className="flex gap-8 max-w-6xl w-full">
        {/* Game Container */}
        <div className="flex-1 bg-white rounded-2xl shadow-2xl p-8 border-4 border-indigo-300 relative">
          <button
          ...
          <div className="mb-6 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-4 border-2 border-indigo-200">
            <div className="flex justify-between items-center">
              <div className="text-lg font-semibold text-gray-700">
                {gamePhase === 'start' && '🎮 Learn & memorize the consonants below'}
                {gamePhase === 'learning' && '📚 Learning Phase - Memorize!'}
                {gamePhase === 'playing' && '🎯 Playing - Arrange the Grid!'}
                {gamePhase === 'completed' && `🎉 Congratulations! Perfect! You completed in ${timeTaken} seconds!`}
        ...
        <div className="w-64 space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border-4 border-indigo-300 space-y-3">
            ...
            {gamePhase === 'playing' && (
              <button
                onClick={learnAgain}
                className="w-full bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white font-bold py-4 px-6 rounded-lg shadow-lg transition-all duration-200 transform hover:scale-105"
              >
                🧠 Learn Again
              </button>
            )}
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border-4 border-indigo-300">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded bg-[#DC143C]"></div>
                <span className="text-sm font-medium">Row 2: ച-ഞ</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded bg-[#FFD700]"></div>
                <span className="text-sm font-medium">Row 3: ട-ണ</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded bg-[#8B4513]"></div>
                <span className="text-sm font-medium">Row 4: ത-ന</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded bg-[#228B22]"></div>
      ...
      {showSuccessOverlay && (
        <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="text-center">
            <img src="/game/parrotclap.gif" alt="Parrot Clapping" className="mx-auto mb-4" />
