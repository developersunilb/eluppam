...
export default function VowelsPracticePage() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [userAnswers, setUserAnswers] = useState<{ [key: number]: string }>({});
  const [isSessionFinished, setIsSessionFinished] = useState(false);
  const [score, setScore] = useState(0);
  const [isKeyboardVisible, setIsKeyboardVisible] = useState(true); // Keyboard visible by default for practice
  ...
  useEffect(() => {
    setShuffledQuestions(shuffle([...practiceVowels]));
  ...
  const handleRestart = () => {
    ...
    setShuffledQuestions(shuffle([...practiceVowels])); // Reshuffle questions
  ...
  return (
    <div className="text-center font-normal">
      ...
      <div className="mt-8">
        <a href="mailto:developersunilb@gmail.com?subject=Feedback on Vowels Practice">
