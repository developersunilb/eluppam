...
export default function CulturalTriviaPage() {
  const router = useRouter();
  const { updateLessonProgress } = useProgress();
  const [gameCompleted, setGameCompleted] = useState(false);
  const [wasSuccessful, setWasSuccessful] = useState(false);

  const currentGameId = 'cultural-trivia';
      ...
      {gameCompleted ? (
        <div className="text-center p-8 bg-white rounded-lg shadow-xl">
          ...
          <p className="text-xl text-gray-700 mb-6">
            {wasSuccessful ? "You've passed the Cultural Trivia game!" : "Better luck next time!"}
          </p>
          <Button 
        ...
        <>
          <h1 className="text-4xl font-bold text-kerala-green-800 mb-8">Cultural Trivia Show</h1>
          <CulturalTriviaGame onComplete={handleGameComplete} />
        </>
      )}
    </div>
  );
